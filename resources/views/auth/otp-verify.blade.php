<x-guest-layout>
    <div class="mb-8">
        ...
        <p class="text-gray-500 text-sm">We've sent a 6-digit verification code to <span class="font-semibold text-gray-900">{{ str_replace(substr($email, 1, strpos($email, '@') - 2), '***', $email) }}</span></p>
    </div>
    ...
    <form method="POST" action="{{ route('otp.verify.submit') }}" id="otp-form" class="space-y-6">
        ...
        <div>
            <div class="flex items-center justify-between mb-4">
                <label class="input-label mb-0">Enter Verification Code</label>
                <span id="countdown" class="text-xs font-semibold text-primary-600 bg-primary-50 px-2 py-1 rounded-md">10:00</span>
            </div>
            ...
            <div class="flex gap-2 sm:gap-3">
                @foreach(range(1, 6) as $i)
                    <input type="text" 
                           maxlength="1" 
                           data-index="{{ $i }}"
                           class="otp-digit-input w-full h-12 sm:h-14 text-center text-xl sm:text-2xl font-bold rounded-xl border border-gray-200 focus:ring-2 focus:ring-primary-500 focus:border-transparent transition-all duration-200 bg-gray-50 uppercase"
    ...
    <div class="mt-8 pt-6 border-t border-gray-100 flex flex-col items-center gap-4">
        ...
        <form method="POST" action="{{ route('otp.resend') }}" id="resend-form">
            ...
            <button type="submit" 
                    id="resend-btn"
                    class="text-sm font-semibold text-primary-600 hover:text-primary-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
                    disabled>
    ...
    <script>
        ...
        const expiryInterval = setInterval(() => {
            ...
            if (expirySeconds <= 0) {
                ...
                countdownEl.className = "text-xs font-semibold text-red-600 bg-red-50 px-2 py-1 rounded-md";
            }
        }, 1000);
