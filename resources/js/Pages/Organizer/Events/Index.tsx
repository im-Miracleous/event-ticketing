    ...
    return (
        <DashboardLayout>
            ...
            <div className="max-w-7xl mx-auto space-y-6">
                ...
                <div className="flex items-center space-x-6 border-b border-white/10 dark:border-white/5 pb-2">
                    <button className="px-4 py-2 text-sm font-bold text-primary-500 border-b-2 border-primary-500">ALL EVENTS</button>
                    <button className="px-4 py-2 text-sm font-bold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">DRAFT</button>
                    <button className="px-4 py-2 text-sm font-bold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">NOW SHOWING</button>
                    <button className="px-4 py-2 text-sm font-bold text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white transition-colors">ENDED</button>
                </div>

                <div className="flex flex-col md:flex-row md:items-center justify-between space-y-4 md:space-y-0 p-6 bg-white dark:bg-navy-900 border border-slate-200 dark:border-white/5 rounded-2xl shadow-sm">
                    <div className="flex items-center space-x-4">
                        <span className="text-slate-700 dark:text-white font-bold px-4 py-2 bg-slate-50 dark:bg-white/5 rounded-xl border border-slate-200 dark:border-white/10">Catatan: Pembuatan event baru hanya dapat dilakukan oleh Admin.</span>
                    </div>
                    ...
                    <div className="relative w-full md:w-96">
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-3 bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500/50 transition-all backdrop-blur-md text-sm font-medium"
                        />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-6">
                    {events && events.length > 0 ? (
                        events.map((event: any) => (
                            <div key={event.id} className="bg-white dark:bg-navy-900 border border-slate-200 dark:border-white/5 rounded-2xl overflow-hidden shadow-sm hover:border-primary-500/30 transition-all group flex flex-col">
                                ...
                                <div className="p-5 flex-1 flex flex-col">
                                    <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors line-clamp-1">{event.title}</h3>
                                    ...
                                    <div className="mt-4 pt-4 border-t border-slate-100 dark:border-white/5 space-y-1">
                                        <div className="flex items-center text-sm">
                                            <span className="font-bold text-slate-900 dark:text-white mr-2">0</span>
                        ...
                        <div className="col-span-full py-16 text-center border-2 border-dashed border-slate-200 dark:border-white/10 rounded-2xl flex flex-col items-center justify-center mt-4 bg-white dark:bg-navy-900/50">
                            ...
                            <p className="text-slate-500 dark:text-slate-400 text-sm max-w-sm mb-6">Looks like you haven't created any events yet, or none match your search criteria.</p>
                            <Link href={route('organizer.dashboard')} className="bg-primary-600 hover:bg-primary-500 text-white font-bold py-2.5 px-6 rounded-xl transition-all shadow-lg shadow-primary-500/25 text-sm">
