...
class ValidationLogController extends Controller
{
    public function index(Request $request)
    {
        ...
        $history = ValidationLog::with(['ticket.detail.transaction.event', 'ticket.ticketType'])
            ->whereIn('ticket_id', Ticket::whereHas('detail.transaction', function($q) use ($eventIds) {
            ...
            ->map(function($log) {
                return [
                    ...
                    'ticket_type' => $log->ticket->ticketType->name ?? 'N/A',
                    'result' => $log->result,
    ...
    public function store(Request $request)
    {
        ...
        $ticket = Ticket::with(['detail.transaction.event', 'ticketType'])
